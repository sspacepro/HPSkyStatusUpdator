namespace HPSkyStatusUpdator.Services;

public static class PetLevelCalculator
{
    private static readonly int[] BaseXp =
    {
        100,110,120,130,145,160,175,190,210,230,
        250,275,300,330,360,400,440,490,540,600,
        660,730,800,880,960,1050,1150,1260,1380,1510,
        1650,1800,1960,2130,2310,2500,2700,2920,3160,3420,
        3700,4000,4350,4750,5200,5700,6300,7000,7800,8700,
        9700,10800,12000,13300,14700,16200,17800,19500,21300,23200,
        25200,27400,29800,32400,35200,38200,41400,44800,48400,52200,
        56200,60400,64800,69400,74200,79200,84700,90700,97200,104200,
        111700,119700,128200,137200,146700,156700,167700,179700,192700,206700,
        221700,237700,254700,272700,291700,311700,333700,357700,383700,411700,
        441700,476700,516700,561700,611700,666700,726700,791700,861700,936700,
        1016700,1101700,1191700,1286700,1386700,1496700,1616700,1746700,1886700
    };

    private static readonly Dictionary<string, int> Offsets = new()
    {
        ["common"] = 0,
        ["uncommon"] = 6,
        ["rare"] = 11,
        ["epic"] = 16,
        ["legendary"] = 20,
        ["mythic"] = 20
    };

    private static int[] GetXpTable(string rarity)
    {
        rarity = rarity.ToLower();

        if (!Offsets.TryGetValue(rarity, out int offset))
            throw new ArgumentException("Invalid rarity.");

        var xp = new int[201];

        xp[1] = 0;

        for (int level = 2; level <= 100; level++)
            xp[level] = BaseXp[offset + level - 2];

        if (rarity == "legendary" || rarity == "mythic")
        {
            xp[101] = 0;
            xp[102] = 5555;

            for (int level = 103; level <= 200; level++)
                xp[level] = xp[100];
        }

        return xp;
    }

    public static long LevelToXp(string rarity, int level)
    {
        if (level <= 1)
            return 0;

        var table = GetXpTable(rarity);

        long total = 0;

        for (int i = 2; i <= level; i++)
            total += table[i];

        return total;
    }

    public static int XpToLevel(string rarity, long xp)
    {
        var table = GetXpTable(rarity);

        long total = 0;
        int currentLevel = 1;

        int maxLevel =
            rarity.Equals("legendary", StringComparison.OrdinalIgnoreCase) ||
            rarity.Equals("mythic", StringComparison.OrdinalIgnoreCase)
                ? 200
                : 100;

        for (int level = 2; level <= maxLevel; level++)
        {
            // Level 101 is not a real displayed level
            if (level == 101)
                continue;

            total += table[level];

            if (xp >= total)
            {
                currentLevel = level;
            }
            else
            {
                break;
            }
        }

        return currentLevel;
    }
}
